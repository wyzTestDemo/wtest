...
import com.example.demo.utils.MWebUtils;
...
public class RestAuthorizationFilter extends PermissionsAuthorizationFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws IOException {
        ...
        if (subject.getPrincipal() == null) {
            // AJAX 请求返回 JSON
            if (MWebUtils.isAjaxRequest(WebUtils.toHttp(request))) {
                ...
                MWebUtils.writeJson(ResultBean.error("未登录"), response);
            } else {
                // 其他请求跳转到登陆页面
            ...
            if (MWebUtils.isAjaxRequest(WebUtils.toHttp(request))) {
                ...
                MWebUtils.writeJson(ResultBean.error("无权限"), response);
            } else {
